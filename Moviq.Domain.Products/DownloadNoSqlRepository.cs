namespace Moviq.Domain.Products
{
    ...
    public class DownloadNoSqlRepository : IRepository<IDownload>
    {
        protected string keyPattern;
        protected string dataType;

        public DownloadNoSqlRepository(IFactory<IDownload> downloadFactory, ICouchbaseClient db, ILocale locale, IRestClient restClient, string searchUrl)
        {
            this.downloadFactory = downloadFactory;
            this.db = db;
            this.locale = locale;
            this.dataType = ((IHelpCategorizeNoSqlData)downloadFactory.GetInstance())._type;
            this.keyPattern = String.Concat(this.dataType, "::{0}");
            this.restClient = restClient;
            this.searchUrl = searchUrl;
        }

        IFactory<IDownload> downloadFactory;
        ICouchbaseClient db;
        ILocale locale;
        ...
        public IDownload Get(string uid)
        {
            return db.GetJson<Download>(String.Format(keyPattern, uid.ToString()));
        }

        private IEnumerable<IDownload> Get(IEnumerable<string> keys) 
        {
            if (!keys.Any())
                yield break;
            ...
            foreach (var o in _results)
                yield return JsonConvert.DeserializeObject<Download>(o.ToString());
        }

        public IDownload Set(IDownload download)
        {
            if (db.StoreJson(StoreMode.Set, String.Format(keyPattern, download.Uid), download))
            {
                return Get(download.Uid);
            }

            throw new Exception(locale.ProductSetFailure);
        }

        public IEnumerable<IDownload> List(int take, int skip)
        {
        ...
        public async Task<IEnumerable<IDownload>> Find(string searchFor)
        {
